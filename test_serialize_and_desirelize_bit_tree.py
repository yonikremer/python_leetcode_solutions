...
class Codec:
    def __init__(self):
        self.d: dict = dict()
        self.n: int = 0  # The index of the last node

    def serialize(self, root: Optional[TreeNode]) -> str:
        start = time.time()
        self.serialize_rec(root, 0)
        ans: str = ""
        ...
        print(f"The time for serialize is: {end-start}")
        return ans

    def serialize_rec(self, root: Optional[TreeNode], i: int):
        if root is None:
            return
        self.n = max(self.n, i)
        self.d[i] = root.val
        self.serialize_rec(root.left, i * 2 + 1)
        self.serialize_rec(root.right, i * 2 + 2)
        return

    def deserialize(self, data: str) -> Optional[TreeNode]:
        start = time.time()
        data_list: List[str] = data.split(',')
        root: Optional[TreeNode] = string_to_node(data_list[0])
        new_root: TreeNode = self.deserialize_rec(root=root, data=data_list, i=0)
        end = time.time()
        print(f"The time for deserialize is: {end - start}")
        return new_root

    def deserialize_rec(self, root: Optional[TreeNode], data: List[str], i: int) -> Optional[TreeNode]:
        if root is None:
            return None
        ...
        if i * 2 + 1 >= len(data):
            ...
            root.left = string_to_node(data[i * 2 + 1])

        if i * 2 + 2 >= len(data):
            root.right = None
        else:
            root.right = string_to_node(data[i * 2 + 2])

        self.deserialize_rec(root=root.left, data=data, i=i * 2 + 1)
        self.deserialize_rec(root=root.right, data=data, i=i * 2 + 2)
        return root
...
def main():
    ...
    print(f"The time to load the data: {time.time()-start}")
