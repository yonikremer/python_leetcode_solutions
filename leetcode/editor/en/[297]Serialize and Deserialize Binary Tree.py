...
class TreeNode(object):
    ...
    def __str__(self):
        """Only used to debug"""
        return f"val: {str(self.val)} left: {node_to_string(self.left)} right: {node_to_string(self.right)}"


def string_to_node(data: str) -> Optional[TreeNode]:
    if data == "null":
        return None
    try:
        x: int = int(data)
...
def node_to_string(node: Optional[TreeNode]) -> str:
    if node is None:
        return "null"
    return str(node.val)
...
class Codec:
    def serialize(self, root: Optional[TreeNode]) -> str:
        if root is None:
            return ""
        ans: str = self.serialize_iter([root])
        while ans.endswith(",null"):
            ans = ans[:-5]
        return ans[1:]

    # def serialize_children(self, root: Optional[TreeNode]) -> str:
    #     if root is None:
    #         return "null"
    #     left: str = node_to_string(root.left)
    #     right: str = node_to_string(root.right)
    #     if root.left is not None and root.right is not None:  # If the root have both sides grandchildren
    #         left_children: str = self.serialize_children(root.left)
    #         right_children: str = self.serialize_children(root.right)
    #         return ",".join([left, right, left_children, right_children])
    #     if root.left is not None and root.right is None:  # If the root have only left side grandchildren
    #         left_children: str = self.serialize_children(root.right)
    #         return ",".join([left, right, left_children])
    #     if root.left is None and root.right is not None:  # If the root has only right grandchildren
    #         right_children: str = self.serialize_children(root.right)
    #         return ",".join([left, right, right_children])
    #     return ",".join([left, right])  # If the root has no grandchildren

    def serialize_iter(self, last_row: List[Optional[TreeNode]]) -> str:
        new_row: List[Optional[TreeNode]] = []
        row_string = "," + ",".join([node_to_string(n) for n in last_row])
        count_nodes: int = 0  # Count the non None nodes is next row
        for node in last_row:
            if node is not None:
                count_nodes += 1
                new_row += [node.left, node.right]
        if count_nodes == 0:
            return row_string
        next_rows_string = self.serialize_iter(new_row)
        row_string += next_rows_string
        return row_string
    ...
    def deserialize(self, data: str) -> Optional[TreeNode]:
        ...
        if len(data_list) == 0:
            return None
        root: Optional[TreeNode] = string_to_node(data_list.pop(0))
        self.deserialize_iter([root], data_list)
        return root

    # def deserialize_rec(self, root: Optional[TreeNode], data_list: List[str]) -> Optional[TreeNode]:
    #     if root is None:
    #         return root
    #     if len(data_list) == 0:
    #         return root
    #     root.left = string_to_node(data_list.pop(0))
    #     if len(data_list) > 0:
    #         root.right = string_to_node(data_list.pop(0))
    #     self.deserialize_rec(root.left, data_list)
    #     self.deserialize_rec(root.right, data_list)
    #     return root

    def deserialize_iter(self, last_row: List[Optional[TreeNode]], data_list: List[str]):
        new_row: List[Optional[TreeNode]] = []
        if len(data_list) == 0:
            return
        for node in last_row:
            if node is not None:
                if len(data_list) > 0:
                    node.left = string_to_node(data_list.pop(0))
                    new_row.append(node.left)
                    if len(data_list) > 0:
                        node.right = string_to_node(data_list.pop(0))
                        new_row.append(node.right)
        self.deserialize_iter(new_row, data_list)
